...
BasisFunctions.Gram(b)
BasisFunctions.eval_element(b,1,0)^2
matrix(BasisFunctions.Gram(b))

matrix(BasisFunctions.DualGram(b))
BasisFunctions.DualGram(b)


@assert norm([quadgk(x->BasisFunctions.eval_element(b,i,x)*BasisFunctions.eval_dualelement(b,j,x),0,1, reltol=1e-6, abstol=1e-6)[1] for i in 1:n, j in 1:n]-matrix(MixedGram(b)))<1e-5
b = FourierBasis(3)
CAO = BasisFunctions.approximation_operator(b; discrete=false)
DAO = BasisFunctions.approximation_operator(b; discrete=true)
f = x->1.
DAO*f
CAO*f

F1 = approximate(b, f)
F2 = approximate(b, f; discrete=false)
coefficients(F1)
coefficients(F2)
...
stepsize(b)
left(set_promote_eltype(b,Complex128),1)
using Plots
n = 10
b=PeriodicBSplineBasis(n,1)
t = linspace(0,1,1000)
