...
"""
abstract Subset{N,T} <: FunctionSet{N,T}
...
"""
immutable LargeSubset{SET, IDX, N, T} <: Subset{N,T}
    superset    ::  SET
    indices     ::  IDX

    function LargeSubset(set::FunctionSet{N,T}, idx::IDX)
        checkbounds(set, idx)
        new(set, idx)
    end
end
...
"""
immutable SmallSubset{SET,IDX,N,T} <: Subset{N,T}
    superset    ::  SET
    indices     ::  IDX

    function SmallSubset(set::FunctionSet{N,T}, indices::IDX)
...
"""
immutable SingletonSubset{SET,IDX,N,T} <: Subset{N,T}
    superset    ::  SET
    index       ::  IDX

    function SingletonSubset(set::FunctionSet{N,T}, index::IDX)
