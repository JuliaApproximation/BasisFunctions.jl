...
function expansion_multiply(src1::Fourier, src2::Fourier, coef_src1, coef_src2)
	T = promote_type(eltype(coef_src1), eltype(coef_src2))
	if oddlength(src1) && evenlength(src2)
	    dsrc2 = resize(src2, length(src2)+1)
	    expansion_multiply(src1, dsrc2, coef_src1, extension(T, src2, dsrc2)*coef_src2)
	elseif evenlength(src1) && oddlength(src2)
		dsrc1 = resize(src1, length(src1)+1)
	    expansion_multiply(dsrc1, src2, extension(T, src1,dsrc1)*coef_src1,coef_src2)
	elseif evenlength(src1) && evenlength(src2)
		dsrc1 = resize(src1, length(src1)+1)
	    dsrc2 = resize(src2, length(src2)+1)
		T1 = eltype(coef_src1)
		T2 = eltype(coef_src2)
	    expansion_multiply(dsrc1,dsrc2,extension(T, src1, dsrc1)*coef_src1, extension(T, src2, dsrc2)*coef_src2)
