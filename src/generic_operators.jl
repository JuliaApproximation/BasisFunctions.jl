    ...
    if has_transform(s)
        ...
        elseif length(s) < length(dgs)
            if ndims(s) == 1
                ...
                has_transform(slarge, dgs) && full_transform_operator(slarge, dgs; options...) * extension_operator(s, slarge; options...)
            ...
            return MultiplicationOperator(s, dgs, evaluation_matrix(s, grid(dgs)))
        else
